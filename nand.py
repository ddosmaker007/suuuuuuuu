...
def approve_or_disapprove_user(message):
    ...
    if action == '/approve':
        if plan == 1 and count_users_with_plan(1) >= 999:
            bot.send_message(chat_id, "*Approval failed: Instant Plan 🧡 limit reached (999 users).*", parse_mode='Markdown')
            return
        elif plan == 2 and count_users_with_plan(2) >= 2000:
            bot.send_message(chat_id, "*Approval failed: Instant++ Plan 💥 limit reached (2000 users).*", parse_mode='Markdown')
            return
...
def process_attack_command(message):
    try:
        ...
        try:
            ...
            join_button = InlineKeyboardButton(text="JOIN NOW❤️‍🔥", url="https://t.me/ZEROSHREK")
